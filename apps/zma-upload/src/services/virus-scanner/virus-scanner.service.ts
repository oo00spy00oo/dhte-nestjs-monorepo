...
export class VirusScannerService implements OnModuleInit, OnModuleDestroy {
  ...
  private readonly clamdEnabled: boolean;
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.clamdEnabled = clamav.enabled;
  ...
  async onModuleInit(): Promise<void> {
    if (!this.clamdEnabled) {
      this.logger.log('🔧 ClamAV scanner is disabled in configuration.');
      return;
    }
  ...
  async scanStream(stream: Readable): Promise<VirusScanOutput> {
    if (!this.clamdEnabled) {
      return { isClean: true }; // Skip scanning when disabled
    }
  ...
  isScannerReady(): boolean {
    return this.clamdEnabled ? this.isInitialized : true;
  }
}
